// https://github.com/AlmosAugusto/inv_zoo_functions
const zoo = {
    animals: [
      {
...
zoo.animals.reduce((acc,animalType) => {
    return acc + animalType.residents.length
},0)

//Buscador de empleado por id
const buscaEmpleado = (id) => {
    return zoo.employees.find(employee => employee.id === id)
}

console.log(buscaEmpleado('56d43ba3-a5a7-40f6-8dd7-cbb05082383f'))

//Buscador de empleados por id
const buscaEmpleados = (ids) => {
    return zoo.employees.filter(employee => {
        return ids.includes(employee.id)
    })
}
console.log(buscaEmpleado(['0e7b460e-acf4-4e17-bcb3-ee472265db83', 'fdb2543b-5662-46a7-badc-93d960fdc0a8', '9e7d4524-363c-416a-8759-8aa7e50c0992']))

// Sacar los nombres de todos los empleados
let nombres = []
zoo.animals.forEach(animalType => {
     const typeNames = animalType.residents.map(resident => resident.name)
     nombres = [...nombres, ...typeNames] // concatena typeNames a names
     // nombres = nombres.concat(typeNames)
     // nombres.push(...typeNames)
})
console.log(nombres)
...
const ticketTotalCalculator = (info, zoo) => {
    /*
    //keys
    const keys = Object.keys(info)
    
    return keys.reduce((acc, key) => {
        return acc + (info[key] * zoo.prices[key])
    },0)
    */
    ...
    return entries.reduce((acc, entry) => {
        const [type, numberOfType] = entry
        return acc + (numberOfType * zoo.prices[type])
    },0)
}
console.log(ticketTotalCalculator(info))